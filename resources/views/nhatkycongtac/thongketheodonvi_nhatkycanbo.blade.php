<style>
    ...
    <div class="center" style="page-break-before: always; font-weight: bold;">THỐNG KÊ NHẬT KÝ CÁN BỘ THEO ĐƠN VỊ</div>
    <div class="center">{{ $tendonvi }} từ ngày {{ $tungay_d_m_Y }} đến {{ $denngay_d_m_Y }}</div>
    <br>
    <p>Tổng số ngày cần cập nhật: {{ count($list_ngay_not_cuoituan) }}. (không tính Thứ 7, Chủ nhật)</p>
    @foreach ($list_canbo_group_by_doi as $tendoi => $thongtindoi)
        <p>
            <div><b>{{ $tendoi }}:</b></div>
            @foreach ($thongtindoi as $canbo)
                @php
